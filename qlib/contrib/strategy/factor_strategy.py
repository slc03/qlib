...
class MyTopkDropoutStrategy(BaseSignalStrategy):
    ...
    def generate_trade_decision(self, execute_result=None):
        ...
        print(f"[Buy Candidate] buy={list(today)}")
...
class OpenAndCloseStrategy(BaseSignalStrategy):
    """开仓和平仓都通过信号控制"""
    def __init__(
        self,
        *,
    ...
    def generate_trade_decision(self, execute_result=None):
        ...
        time_per_step = self.trade_calendar.get_freq()
        for code, score in sorted_series.items():
            code_df = current_temp.get_stock_position_info(code, trade_start_time, time_per_step, ["$open", "$close"])
            if len(code_df) <= 2:
                continue
            last_row = code_df.iloc[-1]
            if last_row['$open'] >= last_row['$close']:
                sell.append(code)
        sell = pd.Index(sell)
        print(f"[Sell Candidate] sell={list(sell)}")
        ...
        if self.method_buy == "top":
            ...
            today = get_first_n(
                pred_score[~pred_score.index.isin(last)].sort_values(ascending=False).index,
                len(sell) + self.hold_count - len(last),
            )
        elif self.method_buy == "random":
            print("[Buy Method] random")
            topk_candi = get_first_n(pred_score.sort_values(ascending=False).index, self.topk)
            candi = list(filter(lambda x: x not in last, topk_candi))
            n = len(sell) + self.hold_count - len(last)
            try:
                today = np.random.choice(candi, n, replace=False)
            except ValueError:
                today = candi
            ...
            raise NotImplementedError(f"[Error] method_buy={self.method_buy} 不支持")
        buy = today[ : len(sell) + self.hold_count - len(last)]
        print(f"[Buy Candidate] buy={list(buy)}")
